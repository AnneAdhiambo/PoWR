...
import { Bookmark, Plus, Trash, Users, X } from "phosphor-react";
...
export default function RecruiterSavedPage() {
  ...
  const [activePool, setActivePool] = useState<TalentList | null>(null);
  const [members, setMembers] = useState<Array<{ developer_username: string; added_at: string; source: string }>>([]);
  ...
  async function openPool(pool: TalentList) {
    setActivePool(pool);
    setMembersLoading(true);
    try {
      setMembers((await recruiterApiClient.getPoolMembers(pool.id)).members as any);
    } catch (loadError: unknown) {
      toast.error(errorMessage(loadError, "Could not load talent-list members"));
    } finally {
  ...
  async function removeMember(username: string) {
    ...
    try {
      await recruiterApiClient.removeFromPool(activePool.id, username);
      setMembers((current) => current.filter((member) => member.developer_username !== username));
      setPools((current) => current.map((pool) => pool.id === activePool.id ? { ...pool, member_count: Math.max(0, (pool.member_count || 0) - 1) } : pool));
      toast.success("Developer removed");
    } catch (removeError: unknown) {
      toast.error(errorMessage(removeError, "Could not remove developer"));
    }
  }
  ...
  return (
    <RecruiterPage>
      ...
      {activePool && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><Card className="w-full max-w-2xl p-6"><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-wider text-gray-500">Shared talent list</p><h2 className="mt-1 text-xl font-semibold text-white">{activePool.name}</h2></div><button onClick={() => setActivePool(null)} className="text-gray-500 hover:text-white"><X size={20} /></button></div><div className="mt-6 space-y-2">{membersLoading ? <LoadingState label="Loading members" /> : members.length === 0 ? <EmptyState title="No developers yet" description="Add developers from Talent Search." /> : members.map((member) => <div key={member.developer_username} className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] p-4"><div><p className="font-medium text-white">@{member.developer_username}</p><p className="mt-1 text-xs text-gray-500">Added {new Date(member.added_at).toLocaleDateString()} · {member.source}</p></div><Button variant="ghost" onClick={() => removeMember(member.developer_username)}><Trash size={15} />Remove</Button></div>)}</div></Card></div>}
    </RecruiterPage>
  );
}
